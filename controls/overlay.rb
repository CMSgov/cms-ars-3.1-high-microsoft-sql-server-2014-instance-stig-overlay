...
include_controls 'microsoft-sql-server-2014-instance-stig-baseline' do
  ...
  control 'V-67893' do
    ...
    desc 'check', 'Review system configuration. If appropriate support staff are not notified immediately upon storage volume utilization reaching 80%, this is a finding.'
    ...
    describe "SQL Server, the operating system, or the storage system must provide a warning to appropriate support staff when allocated audit record storage volume reaches 80% of maximum audit record storage capacity." do
      skip 'This control is manual'
    end
  end

  control 'V-67913' do
    title 'Security-relevant software updates to SQL Server must be installed within the time period directed by CMS policy.'
    ...
    CMS Policy requires correcting identified security-related information system flaws on production equipment within ten (10) business days and all others within thirty (30) calendar days.'
    desc 'check', 'Obtain evidence that software patches are consistently applied to SQL Server, correcting identified security-related information system flaws on production equipment within ten (10) business days and all others within thirty (30) calendar days.
    ...
    desc 'fix', 'Institute and adhere to policies and procedures to ensure that patches are consistently applied to SQL Server, correcting identified security-related information system flaws on production equipment within ten (10) business days and all others within thirty (30) calendar days. '
  end
  ...
  control 'V-67761' do
    desc 'Authentication with a CMS-approved PKI certificate does not necessarily imply authorization to access the SQL Server instance and server-level resources. To mitigate the risk of unauthorized access to sensitive information by entities that have been issued certificates by CMS-approved PKIs, all CMS systems, including SQL Server instances, must be properly configured to implement access control policies. 
  ...
  control 'V-67767' do
    title 'Where SQL Server Audit is in use, SQL Server must allow only the ISSO (or individuals or roles appointed by the ISSO) to select which auditable events are to be audited at the server level'
  end
  ...
  control 'V-67915' do
    desc 'check', 'Obtain evidence that SQL Server software updates are tested before being applied to production servers, and that any exceptions are approved by the ISSO.
  ...
  control 'V-67941' do
    impact 0.0
    desc 'caveat', 'Not applicable for this CMS ARS 3.1 overlay, since the related security control is not incluced in the CMS ARS 3.1'
  end
  ...
  control 'V-67945' do
    ...
    desc 'Windows domain/enterprise authentication and identification must be used (STIG ID: SQL4-00-030300).  Native SQL Server authentication may be used only when circumstances make it unavoidable; and must be documented and AO-approved.

    The CMS standard for authentication is CMS-approved PKI certificates. Authentication based on User ID and Password may be used only when it is not possible to employ a PKI 
